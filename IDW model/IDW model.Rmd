...
myIDW <- function(data = pm25, start_time = "2017-10-2", end_time = "2017-10-30",
  ...
  for (i in seq.POSIXt(as.POSIXct(start_time), as.POSIXct(end_time), "hour")) {
    ...
    colnames(stack[[j]]) <- c("easting_km", "northing_km", "value", "datetime")
...
```{r model run, results="hide"}

krige <- myIDW(xmin = -30, xmax = 30, xinc = 3, ymin = -40, ymax = 20, yinc = 3, mdist = 55, idp = 1.5)
...
krige %>%
  group_by(northing_km, easting_km) %>%
  summarize(s = sum(value)) %>%
  ggplot(aes(easting_km, northing_km)) +
