...
#back to lat/long for qgis
cumul <- cumul %>%
  mutate(raster_y = raster_y/111.005 + 38.5, raster_x = raster_x/87.233 - 122.75)

coordinates(cumul) <- ~raster_x + raster_y

gridded(cumul) <- TRUE

plot(cumul)
points(farms$easting_km, farms$northing_km)

writeRaster(raster(cumul), "local_aq_sensors", format = "GTiff")
